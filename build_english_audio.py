...
async def microsoft_chapter(chapter: dict, number: int, force: bool = False) -> None:
    ...
    for index, text in enumerate(blocks(chapter, number), 1):
        ...
        await edge_tts.Communicate(text, MICROSOFT_VOICE, rate="-8%").save(str(clip))
        clips.append(clip)
...
async def main(provider: str) -> None:
    ...
    for number, chapter in enumerate(chapters, 1):
        if provider in {"microsoft", "all"}:
            jobs.append(microsoft_chapter(chapter, number))
        if provider in {"google", "all"}:
            jobs.append(asyncio.to_thread(google_chapter, chapter, number))
    for start in range(0, len(jobs), 4):
        await asyncio.gather(*jobs[start:start + 4])
    ready = OUTPUT / "ready.js"
    ready.write_text("window.ENGLISH_STUDIO_AUDIO = { microsoftUk: true, googleUk: true };\n", encoding="utf-8")
